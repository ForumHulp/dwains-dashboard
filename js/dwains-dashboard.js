...
(function(){
  ...
  customElements.define = function(name, ctor, opts){
    if (name === 'dwains-homepage-card' && ctor && ctor.prototype && !ctor.prototype.__dd_defined_patched){
      try{
  ...
  if (typeof customElements !== 'undefined') {
    ...
    customElements.define = function(name, constructor, options) {
      if (elementRegistry.has(name) || customElements.get(name)) {
        console.warn('Element already exists:', name);
        return;
      }
      
      try {
        elementRegistry.set(name, { constructor, options, status: 'registering' });
        const result = originalDefine.call(this, name, constructor, options);
        elementRegistry.set(name, { constructor, options, status: 'registered' });
        return result;
      } catch (e) {
        console.error('Registration failed for:', name, e.message);
        elementRegistry.set(name, { constructor, options, status: 'failed' });
        
        // Retry logic
        setTimeout(() => {
          if (!originalGet.call(this, name)) {
            try {
              originalDefine.call(this, name, constructor, options);
              elementRegistry.set(name, { constructor, options, status: 'registered' });
            } catch (e2) {
              console.error('Retry failed:', name);
            }
          }
        }, 50);
      }
  ...
  // Error handler for uncaught cases
  let errorCount = 0;
  window.addEventListener('error', function(e) {
    if (e.message && e.message.includes('Illegal constructor')) {
      errorCount++;
      console.warn(`Custom element error #${errorCount}:`, e.message);
      ...
      if (errorCount >= 4) {
        console.log('Multiple errors, reloading...');
